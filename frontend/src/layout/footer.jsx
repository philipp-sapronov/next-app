...
export const Footer = () => {
  const { phone, email, telegramUsername } = useRecoilValue(configState)
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          ...
          <div className="contacts__grid">
            ...
            <a style={{ display: 'flex', alignItems: 'center' }} href={`mailto://${email}`}>
              <IconWrapper size={20}>{GmailIcon}</IconWrapper>
              <div style={{ marginLeft: 15 }}>{email}</div>
            </a>
            <a
              style={{ display: 'flex', alignItems: 'center' }}
              href={`tel://${ukraineCode}${phone}`}
              target="__blank"
            >
              <IconWrapper size={18}>{PhoneIcon}</IconWrapper>
              <div style={{ marginLeft: 10 }}>{phone}</div>
            </a>
            <div style={{ display: 'flex' }}>
              <a href={`viber://chat/?number=%2B${phone}`} target="__blank">
                <IconWrapper size={22}>{ViberIcon}</IconWrapper>
              </a>
          ...
          <div style={{ opacity: 0.7 }} className="devs__grid">
            <div style={{ fontSize: 14, fontWeight: 200 }}>
              Design by:
              <a className={'--color-green'} style={{ paddingLeft: 10 }} href={designBy.link}>
                {designBy.text}
              </a>
            </div>
            <div style={{ fontSize: 14, fontWeight: 200 }}>
              Production by:
              <a className={'--color-green'} style={{ paddingLeft: 10 }} href={productionBy.link}>
                {productionBy.text}
              </a>
            </div>
          </div>
